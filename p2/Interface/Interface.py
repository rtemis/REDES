...
class Interface:
    ...
    def start_interface(self, args):
        ...
        # Case: Encrypting file
        elif args.encrypt != None:
            # Case: incomplete arguments
            if args.dest_id == None:
                # Print error to terminal
                self.print_arg_error(self.colour.magenta('--encrypt <filename> --dest_id <user_id>'))
            # Case: correct arguments
            else:
                # Get public key of user
                response = self.users.getPublicKey(args.dest_id[0])
                ...
                if response.status_code == 200:
                    ...
                    publicKey = response.json()['publicKey']
                    # Encrypt file
                    self.files.encrypt.encrypt(args.encrypt[0], publicKey)
                # Case: request error
                else:
                    # Print error to terminal
                    print('Error Code: ' + self.colour.red(response.json()['error_code']) + ' Description: ' + response.json()['description'])
        ...
        # Case: Signing file
            ...
            # Error control: parameters
            if (len(args.sign) < 1):
                self.print_arg_error(self.colour.magenta('--sign <fileid>'))
            else:
                # Delete file from user repository
                self.files.encrypt.sign(args.sign[0])
                self.print_ok(' File ' + self.colour.yellow(args.sign[0]) + ' sign...')

        # Case: Signing and encrypting file
        elif args.enc_sign != None:
            # Case: incomplete arguments
            if args.dest_id == None:
                # Print error to terminal
                self.print_arg_error(self.colour.magenta('--enc_sign <filename> --dest_id <user_id>'))
            # Case: correct arguments
            else:
                # Get public key of user
                response = self.users.getPublicKey(args.dest_id[0])
                ...
                if response.status_code == 200:
                    ...
                    self.files.encrypt.sign_encrypt(args.enc_sign[0], publicKey)
                # Case: request error
                else:
                    # Print error to terminal
                    print('Error Code: ' + self.colour.red(response.json()['error_code']) + ' Description: ' + response.json()['description'])
